...
namespace UnityCommon.Collections
{
	...
	public class AVLTree<T>
	{
		...
		public T GetClosestLeaf(T o)
		{
			return GetClosestLeaf(this.Root, o);
		}
		...
		private T GetClosestLeaf(Node<T> node, T o)
		{
			int c = comparer.Compare(o, node.Data);
			...
			if (c < 0)
			{
				return GetClosestLeaf(node.Left, o);
			}
			else if (c > 0)
			{
				return GetClosestLeaf(node.Right, o);
			}
			else
			{
				return GetClosestLeaf(node.Left, o);
