...
namespace UnityCommon.Modules
{
    ...
    [UnityEngine.CreateAssetMenu(fileName = "AnimationModule", menuName = "Modules/Animation")]
    public class AnimationModule : Module<AnimationModule>
    {

        internal Dictionary<Type, Lerper> Lerpers { get; } = new Dictionary<Type, Lerper>
        {
        ...
        public override void OnLateUpdate()
        {
            for (int i = animations.Count - 1; i >= 0; i--)
            {
                ...
                if (animations[i].IsDone)
                {
                    ...
                    {
                        Debug.LogError($"Exception encountered in 'onCompleted' callback of animation: {ex.ToString()}");
                    }
                    finally
                    {
    ...
    public class Interpolator
    {
        ...
        {
            return new Interpolator(t => curve.Evaluate(t));
        }
    ...
    public class Animation<T> : Animation
    {
        ...
        // dynamic requires .NET 4.x, but stripping in IL2CPP has problems with lambda expressions so we cannot use it (2019.1.2 - Check this for later versions). Thus, 'Lerper' implementation for each possible type is written.
        ...
        public Animation(Action<T> setter)
        {
            ...
            lerper = (Lerper<T>)AnimationModule.Instance.Lerpers[typeof(T)];
        }
